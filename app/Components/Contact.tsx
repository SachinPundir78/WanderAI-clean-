  ...
  const sendEmail = (e: React.SyntheticEvent) => {
    ...
    emailjs
      .sendForm(
        "service_qihpiup",
        "template_029lodm",
        formRef.current ?? null, // Add a null check using the optional chaining operator
        "wGq8qh7jrWSR9T8HH"
      )
      .then(() => {
        if (formRef.current) {
      ...
      <div className="relative z-20 max-w-xl w-full text-center">
        ...
        <form ref={formRef} onSubmit={sendEmail} className="mt-10 space-y-4">
          ...
          <textarea
            ...
            required
            rows={4}
            className="w-full resize-none rounded-md bg-black/40 px-4 py-2 text-white placeholder-neutral-400 border border-neutral-700 focus:outline-none focus:ring-2 focus:ring-white/20"
          />
          ...
          {status && (
            <p
              className={`mt-4 text-sm ${status.startsWith("✅") ? "text-green-400" : "text-red-400"}`}
            >
              {status}
            </p>
          )}
